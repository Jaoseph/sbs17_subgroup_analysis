## Diffsig re-try. Sig17+ 5-FU vs Sig17+ Naive treatment
...
FluoroTreat <- clin %>%
  filter(clin$Is.Chemo.Naive == "false", grepl("FU", clin$TR.Chemotherapy.Treatment.Protocol)) %>%
  pull(Sample) 

Naive <- clin %>%
  filter(clin$Is.Chemo.Naive == "true") %>%
  pull(Sample) 

Combined <- c(FluoroTreat, Naive)
...
oac_tally <- oac_tally[oac_tally$Sample %in% Combined,]
...
# check Convergence
pdf("New_Retry/Positive_Naivevs5FUComparisonTraceplot.pdf")
rstan::traceplot(fit, pars="beta")
dev.off()

pdf("New_Retry/Positive_Naivevs5FUComparisonDiffSig.pdf")
diffsig_plot(fit, 
             pars=1:28, 
             ci_level = 80,
