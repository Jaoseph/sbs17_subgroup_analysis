...
icgc_final.sigs <- read.table('Treated_sigProfilerCont.txt', header = TRUE)
...
load('icgc_treated_mt_tally.Rdata') #219 samples
sigs.sbs96_input <- as.data.frame(mt_tally.icgc_treated$SBS_96)
sigs.sbs96_input <- sigs.sbs96_input[,colnames(signatures.sbs96_hg19)]

sigs.id83_input <- as.data.frame(mt_tally.icgc_treated$ID_83)
...
sigs.id83_input <- sigs.id83_input[!rownames(sigs.id83_input) %in% no_ID_samples,] # 211 Samples

# Normalise ID83 counts
icgc_idCounts <- read.table('icgc_treated_indelCounts.txt')
...
print("Saving")
save(sigs_complete,
     file = 'ICGC_Treated_deconstructSigs_Cutoff0.01_SBSandIDnormalised.Rdata')
