...
load('icgc_treated_mt_tally.Rdata')
mut_complete <- as.data.frame(cbind(mt_tally.icgc_treated$SBS_96, mt_tally.icgc_treated$ID_83))

# 2. Load annotation (ann)
load('ICGC_Treated_IDnormalised_PhenotypeAnnotation_clust.Rdata')
...
colnames(mut.dists_mean) <- colnames(mut_byClust[[1]])
rownames(mut.dists_mean) <- names(mut_byClust)

# Save prior clusters
save(mut.dists_mean, file = 'ICGC_Treated_clust_mclust_meanCont.Rdata')
...
ggsave(filename = 'Treated_LikelihoodDistributionsMeans.pdf',
