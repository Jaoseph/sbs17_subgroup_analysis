...
tcga_canonicaldrivers <- data.frame(
  Phenotype_Assigned = heatmap$Phenotype_Assigned,
  Driver = heatmap$symbol)
write.xlsx(tcga_canonicaldrivers, "tcga_canonicaldrivers_allsamples.xlsx", rowNames = FALSE)


#heatmap <- heatmap[heatmap$driver_proportion > .1,] #for canonical, only include drivers >0.1 proportion (NB. chosen as for Naivelike.Sig17+ this means >2 samples, whereas 0.05 means 1 sample has it.)

tcga_canonicaldrivers <- data.frame(
  Phenotype_Assigned = heatmap$Phenotype_Assigned,
  Driver = heatmap$symbol)
write.xlsx(tcga_canonicaldrivers, "tcga_canonicaldrivers_2samples.xlsx", rowNames = FALSE)
...
write.xlsx(heatmap2, "tcga_canonicaldrivers.xlsx", rowNames = TRUE)
...
ht_legend <- Legend(title = "Proportion", col_fun = circlize::colorRamp2(c(0, 1), c("white", "red")))
anno_legends <- list(
...
#pdf("TCGA_Canonical_Driver_Heatmap.pdf", h = 10, w = 7)
...
supp_table <- supp_table %>%
  mutate(priority = ifelse(`Sig17-` == 0 & `NaiveLike.Sig17+` > 0, 1, 2)) %>%
  arrange(priority, desc(`NaiveLike.Sig17+`), desc(`Sig17-`)) %>%
  select(-priority)

# Save
library(openxlsx)
write.xlsx(supp_table, "tcga_canHeatmap_supp_table.xlsx", sheetName = "Table S", rowNames=TRUE)
