...
# Load Primary Samples ID
#samples <- read.csv("/Users/jao/Desktop/MSc_Project/classifier_stratification/classifier_stratified_samples.csv")$Sample
...
# Load Primary Samples Tally Data
...
sigs.id83_input <- oac_tally[,97:179]
...
sigs.sbs96_input <- oac_tally[,1:96]
sigs.sbs96_input <- sigs.sbs96_input[,colnames(signatures.sbs96_hg19)]
sigs.sbs96_input <- sigs.sbs96_input[rownames(sigs.sbs96_input) %in% rownames(sigs.id83_input), ] # 770 samples, with ID data

sigs.sbs96_input <- sigs.sbs96_input[rowSums(sigs.sbs96_input) > 0, ]
sigs.id83_input <- sigs.id83_input[rownames(sigs.id83_input) %in% rownames(sigs.sbs96_input), ] # 721 samples, with ID data

samples <- rownames(sigs.id83_input)
write.table(samples, file = "OCCAMS_SamplewithID.txt", quote = F, row.names = F, col.names = F)
...
primaries_sigs_complete <- cbind(sigs.sbs96, sigs.id83)

print("Saving")
save(primaries_sigs_complete,
     file = 'Primaries_deconstructSigs.Rdata')
